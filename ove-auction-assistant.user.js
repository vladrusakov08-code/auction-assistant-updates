...
(function () {
  ...
  const CARFAX_VALUE_JOB_KEY = 'oveCarfaxValueActiveJobV1';
  const CARFAX_VALUE_LAST_LOOKUP_KEY = 'oveCarfaxValueLastLookupV1';
  const CARFAX_VALUE_CACHE_MS = 24 * 60 * 60 * 1000;
  const CARFAX_VALUE_MIN_INTERVAL_MS = 15000;
  ...
  const SHEET_SAVE_WEB_APP_URL =
    'https://script.google.com/macros/s/AKfycbyK4R9T5iUfKAqfptZa4H-bIQjSRr-9bIbVQE4fJDyOu7xEVgIs7RTROdvj9o5iJWB-/exec';
  ...
  function publicCarfaxValueFromPage(job) {
    ...
    return {
      ...
      records, source:'CARFAX Value', valueUrl:location.href,
      captureMode:'stable-v2', fetchedAt:Date.now(),
    };
  }
  ...
  function runPublicCarfaxValueAutomation() {
    ...
    const tick = () => {
      ...
      if (/^[A-HJ-NPR-Z0-9]{17}$/.test(queryVin) && /^\d{5}$/.test(queryZip) &&
        ...
        job = { vin:queryVin, zip:queryZip, title:'', startedAt:Date.now(), stage:'CARFAX window ready' };
        GM_setValue(CARFAX_VALUE_JOB_KEY, job);
      }
      if (!job || job.completedAt || Date.now() - Number(job.startedAt || 0) > 3 * 60 * 1000) return;
      const pageText = document.body?.innerText || '';
      if (/verification required|captcha|verify you are human|unusual activity|access denied|too many requests/i.test(pageText)) {
        GM_setValue(CARFAX_VALUE_JOB_KEY, {
          ...job, stage:'CARFAX verification required — Open browser ↗',
          error:'CARFAX verification required', code:'captcha', action:'popup',
          popupName:'oveCarfaxValueWindow', completedAt:Date.now(),
        });
        return;
      }
      if (/system error\.?\s*please try again later/i.test(pageText)) {
        GM_setValue(CARFAX_VALUE_JOB_KEY, {
          ...job, stage:'CARFAX temporarily returned a system error',
          error:'CARFAX system error', code:'system_error', completedAt:Date.now(),
        });
        return;
      }
      ...
      if (result) {
        ...
        publishSharedResult(job.vin, { vin:job.vin, title:result.title || job.title || '', mileage:0, color:'' });
      ...
      if (!vinInput || !zipInput || !button || Date.now() - started < 1200) return;
  ...
  async function publishSharedResult(vin, vehicle = {}) {
    ...
    const carfaxValue = GM_getValue(carfaxValueKey(vin), null);
    const signature = JSON.stringify({ vehicle, kbb, carfax, carfaxValue });
      ...
      for (let attempt = 0; attempt < 6; attempt++) {
        try {
          ...
          results[vin] = {
            ...
            carfaxValue:carfaxValue || old.carfaxValue || null,
            updatedAt:Date.now(),
          };
  ...
  async function hydrateSharedResult(vehicle, force = false) {
    ...
    try {
      ...
      const localCarfaxValue = GM_getValue(carfaxValueKey(vehicle.vin), null);
  ...
  function normalizeAuctionDate(month, day, year) {
    const current = new Date();
    let fullYear = Number(year || current.getFullYear());
    const monthNumber = Number(month); const dayNumber = Number(day);
    if (!year && monthNumber < current.getMonth() + 1 - 6) fullYear += 1;
    if (!fullYear || !monthNumber || !dayNumber) return '';
    return `${fullYear}-${String(monthNumber).padStart(2, '0')}-${String(dayNumber).padStart(2, '0')}`;
  }
  function normalizeAuctionTime(hour, minute, meridiem) {
    let value = Number(hour); const suffix = String(meridiem || '').toLowerCase();
  ...
  function readAuctionSchedule() {
    ...
    let date = ''; let time = '';
    const numeric = text.match(/(?:Starts|Sale Date|Auction Date)[^\d]{0,30}(\d{1,2})\/(\d{1,2})(?:\/(\d{2,4}))?\s*(?:-|at)?\s*(\d{1,2})(?::(\d{2}))?\s*(am|pm)/i);
    if (numeric) {
      const year = numeric[3] && Number(numeric[3]) < 100 ? 2000 + Number(numeric[3]) : numeric[3];
      date = normalizeAuctionDate(numeric[1], numeric[2], year);
      time = normalizeAuctionTime(numeric[4], numeric[5], numeric[6]);
    }
    if (!date) {
      const names = { jan:1,feb:2,mar:3,apr:4,may:5,jun:6,jul:7,aug:8,sep:9,oct:10,nov:11,dec:12 };
      const named = text.match(/(?:Sale Date|Auction Date)\s*:?\s*(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun)?\.?,?\s*(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s+(\d{1,2}),?\s+(\d{4})[^\d]{0,10}(\d{1,2})(?::(\d{2}))?\s*(AM|PM)/i);
      if (named) {
        date = normalizeAuctionDate(names[named[1].slice(0, 3).toLowerCase()], named[2], named[3]);
        time = normalizeAuctionTime(named[4], named[5], named[6]);
      }
    }
    return { date, time, lane, laneRun };
  }
  ...
  async function saveCurrentVehicle() {
    ...
    const calculatedDeal = calculateDeal(kbb.values || {}, {
      ...carfax, retailValue:Number(carfax.retailValue || carfaxValue.retailValue || 0),
    }, deal);
    const schedule = readAuctionSchedule();
    const carfaxRetail = Number(carfax.retailValue || carfaxValue.retailValue || 0);
    const carfaxUrl = carfax.reportUrl || carfaxValue.valueUrl ||
      `https://www.carfax.com/value/?oveVin=${encodeURIComponent(vehicle.vin)}&oveZip=${encodeURIComponent(settings().zip)}`;
    const payload = {
        vin:vehicle.vin, title:vehicle.title, mileage:vehicle.mileage, color:vehicle.color,
        auctionSite:AUCTION_SITE, pageUrl:location.href, laneRun:schedule.laneRun, lane:schedule.lane,
        auctionDate:schedule.date, auctionTime:schedule.time,
  ...
  function readDeliveryOrigin() {
    ...
    if (!label) {
      try {
        ...
        label = pageUrl.searchParams.get('location') || pageUrl.searchParams.get('auctionLocation') || '';
      } catch (_) {}
    }
    if (label) {
  ...
  function baseDeliveryPrice(miles) {
    ...
    if (miles <= 100) return 190;
    if (miles <= 250) return Math.max(190, miles * 1.15);
    if (miles <= 500) return Math.max(285, miles * 0.95);
    if (miles <= 750) return Math.max(475, miles * 0.82);
    if (miles <= 1000) return Math.max(615, miles * 0.72);
    if (miles <= 1500) return Math.max(720, miles * 0.62);
    if (miles <= 2000) return Math.max(930, miles * 0.55);
    if (miles <= 2500) return Math.max(1100, miles * 0.50);
    return Math.max(1250, miles * 0.46);
  }
  function calculateDeliveryEstimate(miles, type, isRunning) {
    const multipliers = { sedan:1, suv:1.08, pickup:1.12, van:1.15 };
    const sized = baseDeliveryPrice(miles) * (multipliers[type] || 1);
    const nonRunningFee = isRunning ? 0 : Math.max(125, sized * 0.12);
    return Math.ceil((sized + nonRunningFee) / 25) * 25;
  }
  function feeFromTable(price, rows) {
    const match = rows.find(([limit]) => price <= limit);
  ...
  function copartVirtualBidFee(price) {
    return feeFromTable(price, [
      [99.99,0],[499.99,39],[999.99,49],[1499.99,69],[1999.99,79],
      [3999.99,89],[5999.99,99],[7999.99,119],[Infinity,129],
    ]);
  }
  function copartBuyerFee(price, cleanTitle) {
    const clean = [
      [49.99,25],[99.99,45],[199.99,80],[399.99,120],[499.99,160],
      [599.99,185],[699.99,210],[799.99,230],[899.99,250],[999.99,275],
      [1199.99,325],[1299.99,350],[1399.99,365],[1499.99,380],[1599.99,390],
      [1699.99,410],[1799.99,420],[1999.99,440],[2399.99,470],[2499.99,480],
      [2999.99,500],[3499.99,600],[3999.99,675],[4499.99,710],[5999.99,750],
      [7499.99,800],[7999.99,815],[9999.99,840],[14999.99,850],
    ];
    const nonClean = [
      [49.99,25],[99.99,45],[199.99,80],[299.99,130],[349.99,137.5],
      [399.99,145],[449.99,175],[499.99,185],[549.99,205],[599.99,210],
      [699.99,240],[799.99,270],[899.99,295],[999.99,320],[1199.99,375],
      [1299.99,395],[1399.99,410],[1499.99,430],[1599.99,445],[1699.99,465],
      [1799.99,485],[1999.99,510],[2399.99,535],[2499.99,570],[2999.99,610],
      [3499.99,655],[3999.99,705],[4499.99,725],[4999.99,750],[5499.99,775],
      [5999.99,800],[6499.99,825],[6999.99,845],[7499.99,880],[7999.99,900],
      [8499.99,925],[9999.99,945],[14999.99,1000],
    ];
    if (price >= 15000) return price * (cleanTitle ? 0.0725 : 0.075);
    return feeFromTable(price, cleanTitle ? clean : nonClean);
  }
  function manheimMarketplaceFee(price) {
    return feeFromTable(price, [
      [999.99,40],[2999.99,100],[4999.99,150],[9999.99,175],
  ...
  function readDealSettings(vin) {
    ...
    return {
      ...
      fee: Number(vehicle.fee ?? 0),
      feeManual: Boolean(vehicle.feeManual),
      ...
      input.onchange = () => {
        ...
        if (field === 'targetProfit') {
          ...
          if (field === 'fee') {
            ...
            GM_setValue(key, { ...vehicle, fee:value, feeManual:Boolean(raw) });
  ...
  function bindCarfaxCaptchaButton() {
    document.querySelectorAll('#ove-kbb-content .carfax-captcha-open').forEach((button) => {
      button.onclick = async (event) => {
        ...
        const job = GM_getValue(CARFAX_VALUE_JOB_KEY, null);
        if (job?.action === 'popup') {
          const popup = window.open('', job.popupName || 'oveCarfaxValueWindow');
          popup?.focus();
          return;
        }
        try {
  ...
  function render(message = '') {
    ...
    const carfaxValueJob = GM_getValue(CARFAX_VALUE_JOB_KEY, null);
    const carfaxValueActive = carfaxValueJob?.vin === vehicle.vin && !carfaxValueJob.completedAt;
    ...
    if (!vehicle.vin || !vehicle.mileage) {
      ...
      target.innerHTML = `<div class="manual"><div class="manual-title">Manual vehicle lookup</div>
        ...
        ${vehicle.vin && (carfax || carfaxActive || carfaxRetail || carfaxValueActive) ? `<div class="vehicle-head" style="margin-top:13px"><div class="vehicle">${manualTitle || 'Manual VIN lookup'}</div></div>
    ...
    if (saveButton) {
      ...
      saveButton.title = sheetSaved ? `Saved in ${sheetSaved.sheet || `${AUCTION_SITE} v2`} · row ${sheetSaved.row || ''}` : `Save vehicle to ${AUCTION_SITE} v2`;
  ...
  async function startCarfaxValue(vehicle, zip, force = false) {
    const vin = vehicle.vin.toUpperCase();
    ...
    try {
      const { results } = await readSharedResults();
      const shared = results?.[vin]?.carfaxValue;
      if (!force && fresh(shared)) {
        GM_setValue(carfaxValueKey(vin), shared);
        return { source:'shared cache', value:shared };
      }
    } catch (_) {}
    const active = GM_getValue(CARFAX_VALUE_JOB_KEY, null);
    if (!force && active?.vin === vin && !active.completedAt &&
    ...
    try {
      ...
      GM_setValue(CARFAX_VALUE_JOB_KEY, {
        ...
        error:error.message, code:'popup_error', completedAt:Date.now(),
      });
      render();
  ...
  GM_addValueChangeListener(CARFAX_VALUE_JOB_KEY, () => {
    const current = GM_getValue(CARFAX_VALUE_JOB_KEY, null);
    if (current?.vin && current.completedAt && current.retailValue) {
      publishSharedResult(current.vin, {
        vin:current.vin, title:current.title || '', mileage:0, color:'',
      });
    }
    render();
  });
...
(function () {
  ...
  function publicCarfaxValueFromPage(job) {
    ...
    return {
      vin, zip:job.zip, title:heading,
      retailValue:Number(retail.replace(/,/g, '')),
      records, source:'CARFAX Value', captureMode:'stable-v2', fetchedAt:Date.now(),
    };
  }
